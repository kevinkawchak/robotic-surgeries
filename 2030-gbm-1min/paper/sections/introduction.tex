...
arm), end effector velocity (50 mm/s vs 1,000 mm/s; 20x), end
effector acceleration (200 mm/s^2 vs 10,000 mm/s^2; 50x), joint
angular velocity, E-stop latency (50 ms vs 5 ms; 10x), positioning
accuracy at speed (0.5 mm RMS vs 0.1 mm RMS; 5x), force resolution
(0.01 N vs 0.001 N; 10x), and duty cycle. Render the comparison as
a table using the column type
{>{\raggedright\arraybackslash}p{Xcm}} for each column to avoid large
gaps between words. Cite rosa-one-brain, Lefranc2014ROSAStereotaxy,
...
across 4 arms <= 12 N, per-arm tip force <= 5.0 N, E-stop 5 ms,
heartbeat watchdog 3 ms). Map each arm to its tool assignment: arm 1
hybrid ultrasonic plus waterjet plus pulsed plasma; arm 2 bipolar
coagulation plus irrigation; arm 3 suction plus tissue collection;
arm 4 0.5 T iMRI plus 5-ALA fluorescence plus ultrasound. Cite
claude-opus-47 and claude-code for the 1M token context window that
makes on-premises orchestration tractable. Cite ollama and vllm for
the on-premises inference backends compatible with the comparison
agent. Close the subsection with the practical insight: the on-prem
LLM is a Software as a Medical Device candidate under the FDA SaMD
framework (cite fda-samd). Highlight the safety advantage of single
robot error minimization through 4-arm cross-checking on the
deterministic 1 kHz heartbeat bus.]
