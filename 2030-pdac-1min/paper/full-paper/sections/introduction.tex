...
European Union, with a 5 year overall survival below 13 percent
...
\cite{intuitive-davinci-sp}, the Medtronic Hugo RAS multi arm
platform \cite{medtronic-hugo-ras}, and the Verb Surgical digital
surgery platform \cite{verb-surgical}. None can perform a 60
...
\midrule
  Arms & 1 to 4 \cite{intuitive-davinci-sp, medtronic-hugo-ras, verb-surgical} & 8 (dissection, three anastomoses, hemostasis, drains) \\
...
The thesis is stated verbatim through paraphrase from
...
Repository based grounding closes the audit gap. Every LLM command
is sourced from a hash pinned in repository file under
...
On premises inference backends Ollama \cite{ollama} and vLLM
\cite{vllm} carry the same Claude Code agent stack
\cite{claude-code, claude-opus-47, claude-sonnet-46} used to
generate the four phase workflow in this paper, while preserving
...
The artifacts paired with this paper are the output of a four
...
The Daraxonrasib pairing rationale is anchored in
\texttt{2030-pdac-1min/paper/instructions/daraxonrasib_integration.md}
and the Daraxonrasib LLM trial summary
