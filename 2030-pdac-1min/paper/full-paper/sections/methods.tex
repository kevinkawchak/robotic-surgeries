...
parallel coelomic oncology robot specified in
...
The 8 arm parallel coelomic configuration is visualized in
...
\midrule
  Arm 1 & Hybrid u-w-p endeffector & P1 to P8 (active dissect) & Tip force, joint torque, tool ID \\
  Arm 2 & Hybrid u-w-p endeffector & P1 to P8 (active dissect) & Tip force, joint torque, tool ID \\
  Arm 3 & Bipolar + retractor & P1 to P8 (vessel control) & Tip force, current, retractor force \\
...
Each arm carries 80 channels for a total of 640 channels, specified
in
...
tension, and bile spectrophotometry. Force channels sample at 100
kHz; all other channels sample at the 10 kHz command rate. The
execution tree
...
\centering
\begin{tabular}{>{\raggedright\arraybackslash}p{1.0cm} >{\raggedright\arraybackslash}p{4.0cm} >{\raggedright\arraybackslash}p{2.0cm} >{\raggedright\arraybackslash}p{1.8cm} >{\raggedright\arraybackslash}p{4.0cm}}
\toprule
...
The per arm xyz Cartesian mapping pipeline is specified in
\texttt{2030-pdac-1min/paper/instructions/commit_03_xyz_8arm.md}
and
\texttt{2030-pdac-1min/paper/instructions/multi_arm_coordination_8arm.md}.
The codegen tree authored the Python mapping in
...
artery, celiac axis, superior mesenteric artery) carries three
concentric volumes: a no fly volume that triggers an E stop on
breach, a soft warning volume that triggers an LLM advisory, and a
hard stop volume that triggers a 3 ms platform halt. The Python
implementation in
...
The three anastomosis protocols are defined in
...
The 32 iteration sweep is specified in
...
(per iteration committed budget 980 KB across L1 50 ms aggregates,
...
and Rust runner at \texttt{runner_1min.rs}.

The run output is recorded at
\texttt{2030-pdac-1min/paper/execution/iterations/}: a 32 row
...
The 4 entrant tournament protocol is specified in
...
successor, Hugo RAS oncology successor, Verb Surgical successor)
...
LLM judge emits one rationale per round with the structural time
dimension caveat (1 minute robot versus 4 to 8 hour human baseline)
preserved verbatim. The structured output lands at
...
\midrule
  ...
  Time (60 s vs 4 to 8 h structural delta) & 0.20 & Compression value, structural caveat preserved \\
...
The Daraxonrasib perioperative pause and restart logic is
specified in
...
iteration serum concentration target.
...
numbers are T 0 serum 0.45 ng/mL across all 32 iterations (below
the 0.5 ng/mL trough threshold), postoperative restart advisory
T+7d in 29 of 32 iterations, T+14d in 3 of 32, and T+21d in 0 of
32. The FDA SaMD \cite{fda-samd} framing is preserved verbatim in
every advisory rationale.
