...
- `patient-robot-advocacy/` directory containing the v1.0.0 Patient Robot Advocacy paper, built by Claude Code Opus 5 from the single master prompt filed verbatim at `patient-robot-advocacy/prompts/prompt-patient.md` across 122 commits in eight sequential stages within a single PR. The paper is `Patient Robot Advocacy: A Phase 1, First-in-Human, PDAC Clinical Trial Protocol of a LLM-Directed Robotic Whipple with Daraxonrasib (RMC-6236)`, Draft 1.0, Kevin Kawchak, CEO ChemicalQDevice, San Diego, July 31, 2026, DOI `10.5281/zenodo.xxxxxxxx`, ORCID 0009-0007-5457-8667.
- `patient-robot-advocacy/prompts/` with `prompt-patient.md` (the master prompt, verbatim) and `output-patient.md` (the complete Claude markdown output for the run, including the nine decisions taken before the first sub-prompt was written and a table of every rule in the master prompt against where it is satisfied).
- `patient-robot-advocacy/sub-prompts/` with the eight sub-prompts generated by Process A and executed in order by Process B: `prompt-1-mermaid.md` through `prompt-8-final-patient.md`.
- Thirty machine-readable diagram sources across the five permitted platforms and no others, each in its platform's own syntax and each carrying a header naming the figure, the section it serves, and the concern it answers: `patient-robot-advocacy/mermaid/` (9 `.md` with fenced mermaid), `patient-robot-advocacy/d2/` (7 `.d2`), `patient-robot-advocacy/plantuml/` (5 `.puml`), `patient-robot-advocacy/graphviz/` (5 `.dot`), `patient-robot-advocacy/diagrams-python/` (4 `.py`). No Excalidraw output anywhere.
- `patient-robot-advocacy/draft-patient/` (Stage 6): the thirteen-section scaffold with 78 `\draftinstr{...}` bracketed instructions naming the exact repository file each later stage must read, plus `main.tex`, `patientstyle.sty`, `references.bib`, `README.md`, the sub-prompt, the stage narrative, and an Overleaf archive. Compiles clean at 41 pages with every figure slot empty.
- `patient-robot-advocacy/full-patient/` (Stage 7): every bracketed instruction executed, all thirty figures drawn natively in TikZ, all tables filled, two independent figure verification passes recorded in the stage narrative, 83 pages, and an Overleaf archive.
- `patient-robot-advocacy/final-patient/` (Stage 8): the proof-read source set, 88 pages, thirteen sections, thirty figures in ascending order, forty-three tables, 51 bibliography entries, an Overleaf archive, and a stage narrative listing eight defects before fixing any of them. There is no `publication/` subdirectory, per the master prompt.
- Fifteen directory READMEs, each with badges and each stating which files from which other directories it used and where.
- `patient-robot-advocacy/inputs/`, `references/`, `research/`, and `template/` with READMEs covering the five author source documents, the author's `references.bib`, the two dated July 28, 2026 patient-concern research passes by Gemini 3.1 Pro and ChatGPT 5.6 Thinking Extended, and the trial-protocol-template workflow the build adapts.
- Two new sections in the top level `README.md`, which is the limit the master prompt sets: `v1.0.0 Patient Robot Advocacy Paper (ASCII)` with the 425-character summary, the eight-stage ASCII panel, and two tables; and `Patient Robot Advocacy Figure Index and Contents (v1.0.0)` with the thirty-row figure index and the thirteen-section contents.
- v1.0.0 release badge, Patient Advocacy badge, Advocacy Figures badge, and Advocacy DOI badge in the top level `README.md`.
- `patient-robot-advocacy/` subtree in the top level `README.md` Repository Structure block.
- Root `.gitignore` covering LaTeX auxiliary output and `__pycache__`.
- This v1.0.0 entry in `CHANGELOG.md` plus the matching v1.0.0 block in `releases.md`.

### Changed

- Top level `README.md` overview leads with v1.0.0 and keeps v0.9.0 as the prior release; the citation block is bumped to v1.0.0.
- `releases.md` prepended with the v1.0.0 release notes block per the FORMAT (Release title / Summary / Features / Contributors / Notes), and its header paragraph extended with the v1.0.0 sentence.
- Figure numbering harmonised repository-wide so that all thirty figures appear in ascending order: 20 exchanged with 21 and 24 with 25, applied to the paper captions, fifteen prose and table cross-references, six short-form `Fig` labels inside Figure 5, the figure inventory, the cover panel, and the four diagram source files, which were renamed so each still carries the number of the figure it draws. `graphviz/fig-20-hazard-barrier-fault-tree.dot` became `fig-21-...`, `d2/fig-21-force-nofly-envelope.d2` became `fig-20-...`, `diagrams-python/fig_24_data_pipeline.py` became `fig_25_...`, and `mermaid/fig-25-visit-timeline.md` became `fig-24-...`. Per-type counts are unchanged because each swap exchanges one figure each way.
- `patient-robot-advocacy/full-patient/README.md` gains a note recording the four swaps rather than being renumbered, because Stage 7's PDF and archive were built under the old numbering and are internally consistent.
- Fifteen subsections added across the thirteen sections in Stage 8 to bring the paper to length parity with the parent protocol, taking it from 129,078 to 168,275 visible text characters against the protocol's 155,222.
- @kevinkawchak supplied the master prompt, the five input documents, the author `references.bib`, and the two research passes on 2026-07-30.
- @claude (this session) authored the eight-stage build at `patient-robot-advocacy/` across 122 commits within a single PR on 2026-07-30.
- @google-gemini produced the six-family patient-concern research pass at `patient-robot-advocacy/research/research-a.md` on 2026-07-28.
- @openai produced the sixteen-concern patient research pass with its thirteen supporting references at `patient-robot-advocacy/research/research-b.md` on 2026-07-28.

### Fixed

- Figures set inline stranded the space they could not fit into, leaving a trailing gap over 3 cm on 41 of 83 pages, the worst 21.6 cm. Every figure-plus-caption is now carried by a `pafloat`, so running text closes the page and the figure heads the next one. Pages with such a gap fall to 15 of 88, eleven of which are a section's last page and follow from the mandated `\clearpage` between sections.
- Two unbreakable `tabularx` tables printed content past the bottom margin, by 143 pt in the worst case, because `\@nobreak` after a heading prevented the break. All forty-three tables are now `xltabular` with a repeated header, so the document contains zero unbreakable boxes.
- Two headings were the last thing on their page and overfilled it. `\subsection` now reserves itself plus three lines and `\subsubsection` plus two.
- The contents spilled two entries onto a fourth page that was then 21 cm empty. `\l@section` redefined with a 0.18em lead and the contents set `\footnotesize` at `\linespread{0.92}`, fitting three pages with no entry removed.
- Eleven cross-references pointed at the wrong subsection once the new subsections renumbered sections 1, 2, 3, and 12. All 59 distinct subsection references now resolve.
- All thirty captions rebalanced to three lines of near-equal character count; five were reworded because no split of the existing wording could reach the band.
- Three figures selected their fills with a conditional inside a `\foreach`, which TeX never evaluates as the source implies, so every cell rendered in the default fill. All three rebuilt with explicit per-item fill tokens.
- The figure spacing rule made exact: the frame-to-caption distance is 6.1 pt for all thirty figures, floating or inline, because every skip in the figure environment is rigid and `\figcaption` opens with `\nointerlineskip`.
- Single dash invariant verified across the paper and the repository documentation: zero em dashes, zero en dashes, zero prose double or triple dashes. Zero occurrences of `SS` where `\S` was meant.
- Zero PNG or JPG files and zero `\includegraphics` calls anywhere in `patient-robot-advocacy/`; the twenty pictograms that stand in for raster node icons are TikZ line art.
- Palette compliance: eleven colour tokens exist and no twelfth is used anywhere.
- `pdflatex -> bibtex -> pdflatex -> pdflatex` on all three paper stages: 0 errors, 0 overfull boxes, 0 undefined citations, 0 undefined references.
