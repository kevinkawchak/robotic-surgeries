...
\texttt{inputs/phase-1-trial-protocol.zip}, \texttt{sections/sec-01-summary.tex}
...
\texttt{inputs/cancer-patient-journey.zip} and \texttt{inputs/README.md}.}

\begin{pafig}[mermaid-type: flowchart LR, full page]
\node[mmgoal] (a) at (0,0) {Figure 3 slot\\journey schema\\full page, five lanes};
\node[mmin] (b) at (0,-2.2) {Source:\\mermaid/fig-03-journey-schema.md};
\draw[mmedge] (a) -- (b);
\end{pafig}
\vspace{-0.7cm}
...
from \texttt{mermaid/fig-03-journey-schema.md}. Five \texttt{mmlane} lanes at
...
\texttt{sections/sec-01-summary.tex} \S1.3, and convert every cross in that
...
\node[d2box] (b) at (0,-1.8) {Source:\\d2/fig-04-soa-patient-grid.d2};
\draw[d2edge] (a) -- (b);
\end{pafig}
\vspace{-0.7cm}
...
\texttt{d2/fig-04-soa-patient-grid.d2}: eight columns by thirteen rows of
...
from \texttt{inputs/phase-1-trial-protocol.zip},
\texttt{sections/sec-01-summary.tex} \S1.3, unchanged.}
...
the paper's credibility and must not be trimmed in \texttt{full-patient}.}
