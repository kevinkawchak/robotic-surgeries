...
endpoint, with every node the participant controls filled Corporate Blue and every node\\
the sponsor controls left white, so that the balance of control is visible.}
\end{pafloat}
...
chair, with each cell stating what they do rather than what is collected, and a totals\\
row giving the approximate hours of their time each visit will consume.}
\end{pafloat}
...
DL1 and DL2, and \S 5.6 states what happens if one of those observations goes
...
in June. \S 6.2 states the criteria that are re-checked, and \S 6.4 states what
