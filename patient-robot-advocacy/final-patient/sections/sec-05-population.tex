...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 14. Eligibility drawn as two decision columns running against each other, what the\\
protocol asks of the participant and what the participant may ask of the protocol,\\
converging on a single enrollment node that requires both columns to be satisfied.}
\end{pafloat}

\vspace{0.30em}

\begin{xltabular}{\textwidth}{L{3.5cm} L{5.0cm} Y}
\toprule
...
\begin{xltabular}{\textwidth}{L{5.4cm} Y}
\toprule
...
\begin{xltabular}{\textwidth}{L{4.7cm} L{3.4cm} Y}
\toprule
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 15. Booking a screening slot directly and receiving an answer from a named human\\
at the sponsor, with activation bars showing who is blocked at each step and every branch\\
of the eligibility fragment notifying the participant's own oncologist.}
\end{pafloat}
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 16. The five choices the participant keeps, revealed as accumulating layers with\\
the choice, the interval in which it is live, and what happens if the answer is no, so\\
that no decision that matters is taken at the moment of least capacity.}
\end{pafloat}
...
\begin{xltabular}{\textwidth}{C{0.8cm} L{4.9cm} Y}
\toprule
...
\subsection{Second opinions, and how to get one that is worth having}

Nothing in this protocol discourages a second opinion, and \S 8 makes declining
after consent free of consequence. The practical question is not whether to seek
one but how to make it useful, because a second opinion given without the
protocol in front of it is an opinion about robotic surgery in general rather
than about this study.

Three documents make the difference, and the site will provide all three on
request: the protocol synopsis, the Phase 0 gate sign-off with its Unified
Safety Level, and the eligibility determination that names which criteria the
participant meets and which were borderline. A surgeon reading those three can
answer the question the participant is actually asking, which is whether this
operation at this site is a reasonable choice for this disease.

Two questions are worth putting to the second opinion directly. Whether the
tumour is resectable on the imaging as read by an independent radiologist, since
resectability is the eligibility criterion most open to interpretation and \S 7.2
excludes borderline disease. And whether the participant's own comorbidity
profile makes a longer operative time a material risk, since a platform-assisted
Whipple is not shorter than a conventional one during a learning phase.

A second opinion that disagrees is not a problem for the study. It is
information the participant is entitled to have before consent rather than
after, and a site that treats disagreement as an obstacle has misunderstood
which party the protocol serves.

\subsection{Bringing someone with you, and who may decide}

Consent is a conversation, and a participant may bring anyone they choose to it.
The protocol places no limit on that, and the site is expected to schedule the
conversation so that a family member or advocate can attend rather than at
whatever time is convenient for the clinic.

The legal position is narrower and should be stated exactly. Consent for this
study must be given by the participant personally. There is no provision for a
legally authorised representative, for the reason that the eligibility criteria
already require an ECOG performance status of 0 or 1 and full decisional
capacity, so the situation in which a representative would be needed is one in
which the participant is not eligible. A participant who loses capacity after
enrollment does not lose the protections of the study, and \S 8.4 states how
their data and their withdrawal rights are handled in that case.

A companion may ask questions, may take notes, and may hold the copy of this
paper. What a companion may not do is consent on the participant's behalf, and a
site that accepts a companion's signature has committed a deviation that is
reportable under \S 11.6.

Interpretation is provided where it is needed, by a qualified medical
interpreter rather than by a family member, because a family member translating
a consent conversation is being asked to carry both roles at once and neither is
well served.
