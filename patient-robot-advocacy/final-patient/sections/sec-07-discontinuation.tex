...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 22. The four routes out of the study drawn as a swimlaned activity, with one\\
lane for the participant, one for the site team, and one for the data and specimens,\\
so each route's consequence for the record is visible directly beside the route itself.}
\end{pafloat}
...
reachable from every state, and a re-consent transition the participant did not\\
cause, triggered by a software version change, a new risk, or a protocol amendment.}
\end{pafloat}
