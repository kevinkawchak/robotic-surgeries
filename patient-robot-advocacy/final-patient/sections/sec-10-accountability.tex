...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 29. The responsibility matrix with exactly one accountable party in every row\\
and a final column, the only black fill in the figure, naming the person the participant\\
should actually call when that particular thing goes wrong for them.}
\end{pafloat}

\vspace{0.30em}

\begin{xltabular}{\textwidth}{L{4.6cm} L{3.7cm} Y}
\toprule
...
\begin{xltabular}{\textwidth}{L{4.1cm} L{4.2cm} Y}
\toprule
...
\begin{xltabular}{\textwidth}{L{4.5cm} L{4.0cm} Y}
\toprule
...
\subsection{What accountability does not mean here}

The word accountability is used in this section in a narrow sense, and the
narrowness is deliberate, because the broad sense promises something no protocol
can deliver.

It does not mean that someone will be punished. A named accountable party is a
person who must answer for a failure class, explain what happened, and carry the
decision about what changes. Whether anyone is disciplined is a matter for the
institution's own processes and for the law, and this protocol has no authority
over either. A participant told that a protocol makes someone accountable, who
then discovers that accountable meant only that a name appears in a matrix, has
been oversold.

It does not mean compensation. \S 12.3 states what the sponsor pays and what
remains unsettled, and no clause in the responsibility matrix creates a right to
payment. Injury compensation in a trial is governed by the site's own agreement
and by the consent form, and a participant should read that paragraph of the
consent form specifically rather than infer it from this section.

It does not mean that the accountable party was at fault. The matrix assigns
answerability in advance, before anyone knows what will happen, precisely so
that the assignment is not made afterwards by whoever is least able to object.
The party accountable for a device failure class answers for it whether the
failure was foreseeable or not.

What it does mean is that for every failure class there is exactly one party who
must answer, that the party is named before the study opens rather than found
afterwards, that no party reviews its own work, and that the participant has a
route to that party which does not run through the person who recruited them.
Those four properties are checkable, and Figure 29 is the check.
