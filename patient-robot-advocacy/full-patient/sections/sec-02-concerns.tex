...
\noindent\begin{tabularx}{\textwidth}{L{4.2cm} C{1.9cm} Y}
\toprule
...
% ---- family 2 ----
\node[d2key,text width=26mm] (s1) at (5.4,0)    {6. Unintended motion\\\S7.4, Fig 20};
\node[d2box,text width=26mm] (s2) at (5.4,-1.5) {7. Misreading a margin\\\S7.5, Fig 20};
\node[d2key,text width=26mm] (s3) at (5.4,-3.0) {8. Vessel injury\\\S7.5, Fig 21};
\node[d2box,text width=26mm] (s4) at (5.4,-4.5) {9. Instrument failure\\\S7.6, Fig 20};
...
% ---- family 4 ----
\node[d2key,text width=26mm] (d1) at (0,-9.4)   {15. Who sees the video\\\S9.3, Fig 24};
\node[d2box,text width=26mm] (d2) at (0,-10.9)  {16. Training other models\\\S9.3, Fig 24};
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 5. The twenty-one documented concerns grouped into six disjoint families,\\
with the answering section and figure printed inside each item, and a fill key\\
stating how many are answered by a limit, a guarantee, or by governance alone.}
...
and nothing crosses during. Figure 17 draws that boundary and Figure 24 draws
...
a statement about something that does not happen, and Figure 24 renders it as an
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 7. Sixteen documented concerns positioned by how often they are raised in\\
the surveyed literature against how completely this protocol answers them, with the\\
lower-right quadrant collecting the concerns answered by governance or disclosure alone.}
...
with the participant as the primary actor, so that each one reads as a capability\\
they hold rather than as an obligation the sponsor has undertaken on their behalf.}
...
bodies, and everything outside, with the intra-procedural route drawn and struck through.}
...
16 & Training other models & Will my data be used to train other systems? & Not without a separate consent; drawn in Figure 24 as an absent edge. Procedural guarantee. \\
