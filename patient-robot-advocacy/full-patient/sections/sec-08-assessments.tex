...
Figure 25 converts the Schedule of Activities into a calendar with durations,
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 25. The participant calendar from screening through the 24-month endpoint,\\
drawn with real durations rather than crosses in a matrix, and with the four critical\\
...
\draw[dgedgeb] (u1) to[out=90,in=-135,looseness=0.85] (s1);
\draw[dgedged] (u2) to[out=90,in=-140,looseness=0.85] (s1);
\draw[dgedged] (u3) to[out=90,in=-120,looseness=0.85] (s3);
\draw[dgedged] (u4) to[out=90,in=-60,looseness=0.85] (s3);
\draw[dgedged] (u5) to[out=120,in=-60,looseness=0.85] (q1);
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 24. Your data through four stages, capture, a write-once hash chain, the identified\\
and de-identified stores, and what leaves, with the read list attached to every store\\
and no edge at all drawn for a principal who may not read that particular store.}
...
consent. That is a statement about something that does not happen, and Figure 24
...
\end{pafig}
\vspace{-0.7cm}
\figcaption{Figure 26. Ten robot-type instruction cards re-scoped from general oncology to this\\
PDAC Whipple protocol, each with three numbered instructions and a re-scoping note,\\
and with the four robot types this protocol does not use marked plainly as unused.}
